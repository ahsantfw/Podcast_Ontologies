...
class PodcastAgent:
    ...
    DOMAIN:
    - 🧠 Philosophy: Deep thinking, wisdom, mental models, meaning of life
    - 🎨 Creativity: Innovation, artistic process, creative practices, inspiration
    - 🏆 Coaching: Leadership, performance, team dynamics, personal growth
    - 🧘 Personal Development: Mindfulness, meditation, self-awareness, well-being
    
    CAPABILITIES:
    ✅ Answer questions about concepts, people, and ideas from podcasts
    ✅ Find relationships between different concepts (Practice → Outcome)
    ✅ Retrieve relevant quotes with full source attribution (episode, speaker, timestamp)
    ✅ Discover patterns across multiple episodes (cross-episode analysis)
    ✅ Multi-hop reasoning (X → Y → Z connections)
    ✅ Speaker-anchored queries ("What did Phil Jackson say about...?")
    ...
    ❌ General world knowledge outside podcast corpus
    ❌ Weather, sports scores, stock prices
    ❌ How to write code or debug programs
    ❌ Recipes, geography facts, general trivia
...
4. If query asks about specific relationship types ("what is ABOUT?", "show ENABLES") → kg_query
...
- "what are all relationships" → list_kg  
- "what is the ABOUT relationship" → kg_query (asking about relationship TYPE)
- "show me ENABLES connections" → kg_query (asking about relationship TYPE)
- "what practices are most associated with clarity" → knowledge_query (asking about CONTENT)
- "what concepts appear across multiple episodes" → knowledge_query (asking about CONTENT)
- "what did Phil Jackson say about creativity" → knowledge_query (asking about CONTENT)
- "what is creativity" → knowledge_query (asking about concept from podcasts)
- "how many transcripts/episodes/concepts" → knowledge_query (asking about data/counts)
- "who is the PM of Pakistan" → Already filtered (out of scope)
...
{{"node_type": "...", "limit": 20, "filter": null}}"""
        ...
        try:
            if node_type.lower() == "all":
                ...
                answer_parts = ["## 📚 Knowledge Graph Contents"]
                ...
                # Format response with proper markdown
                answer_parts = [f"## 📚 {node_type} Nodes ({len(results)} shown)"]
                answer_parts.append("")
                for r in results:
...
Provide a clear, well-formatted answer:"""
    ...
    def _format_coverage_status(self, coverage_info: Dict[str, Any]) -> str:
        ...
        return f"COVERAGE STATUS:\n- ✅ HAVE sources for: {covered}\n- ❌ MISSING sources for: {missing}\n\n"
    ...
    def _extract_mentioned_entities(self, query: str) -> List[str]:
        ...
        known_people = {
            "phil jackson", "jerrod carmichael", "iñárritu", "alejandro inarritu", 
    ...
    def _handle_knowledge_query(
        ...
        if mentioned_entities and len(mentioned_entities) > 1:
            ...
            if filtered_rag:
                self.logger.info(f"Filtered RAG results: {len(rag_results)} → {len(filtered_rag)} (only mentioned entities)")
...
"For Phil Jackson and Iñárritu, [principle]. However, I don't have sufficient sources from Jerrod Carmichael in the knowledge base to confirm this principle. Therefore, I can only establish partial overlap."
